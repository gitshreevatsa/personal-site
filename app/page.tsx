...
function AnimatedEmail({ cycleIdx }: { cycleIdx: number }) {
  ...
  return (
    <a
      ...
      style={{ color: '#555', fontSize: 12, display: 'flex', alignItems: 'center', gap: 0 }}
    >
...
function HeroTagline({ cycleIdx }: { cycleIdx: number }) {
  ...
  return (
    ...
    >
      {"hi, i'm "}
      <span style={{ fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: '1.08em' }}>
        shreyas
      </span>
      {", "}
      {displayed}
...
const JOBS = [
  {
    ...
    tags: ['TypeScript', 'Solidity', 'LayerZero', 'Wormhole', 'Hardhat', 'Foundry', 'BullMQ', 'Redis'],
  },
  {
  ...
  },
  {
    ...
    tags: ['TypeScript', 'Solidity', 'WebSocket', 'REST APIs', 'Docker', 'Oracles', 'DeFi'],
  },
];
    ...
    >
      <h2
        style={{
          ...
          fontSize: 'clamp(36px, 5vw, 56px)', color: '#e8e8e8', marginBottom: 52,
          textDecoration: 'underline', textUnderlineOffset: 8, textDecorationColor: '#333',
        }}
      >
        Experience
      </h2>
      ...
      <div style={{ width: '100%', maxWidth: 880, display: 'flex', flexDirection: 'column' }}>
        {JOBS.map((job, i) => (
          <div key={i} style={{ display: 'flex', gap: 20, marginBottom: i < JOBS.length - 1 ? 32 : 0 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 6, flexShrink: 0 }}>
              <div
                style={{
                  width: 30, height: 30, background: '#161616', border: '1px solid #2a2a2a',
              ...
              {i < JOBS.length - 1 && (
                <div style={{ width: 1, flex: 1, background: '#1e1e1e', minHeight: 40, marginTop: 4 }} />
              )}
            </div>

            <div style={{ flex: 1 }}>
              <div style={{ border: '1px solid #1e1e1e', padding: '20px 24px', background: '#0d0d0d', marginBottom: 12 }}>
                <p style={{ color: '#e0e0e0', fontSize: 14, marginBottom: 6 }}>
                  {job.title} <span style={{ color: '#555' }}>@</span>{' '}
                  <span style={{ color: '#ccc' }}>{job.company}</span>
                </p>
                <p style={{ color: '#555', fontSize: 12, marginBottom: 16 }}>{job.type} · {job.period}</p>
                <p style={{ color: '#777', fontSize: 12, lineHeight: 1.85 }}>{job.description}</p>
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {job.tags.map((tag) => (
                  <span key={tag} style={{ border: '1px solid #252525', color: '#555', fontSize: 11, padding: '4px 10px' }}>
                    {tag}
                  </span>
                ))}
              </div>
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 520, width: '100%' }}>
        <p style={{ color: '#333', fontSize: 11, marginBottom: 24, letterSpacing: '0.1em' }}>// about</p>

        <div style={{ border: '1px solid #1a1a1a', padding: '24px 28px', background: '#0d0d0d', marginBottom: 28 }}>
          {[
            ['handle', 'shreyaspadmakiran'],
            ['alias', 'sakai_thezkguy'],
            ['location', 'bangalore, india'],
          ].map(([k, v]) => (
            <div key={k} style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 12, fontSize: 12 }}>
        ...
        <p style={{ color: '#555', fontSize: 12, lineHeight: 1.9, marginBottom: 32 }}>
          Solutions engineer with 3+ years building production infrastructure across EVM,
          cross-chain protocols, and DeFi. Specialize in making complex integrations
          frictionless — from architecture review to mainnet deployment.
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 32 }}>
          {[
            ['quilcheck', '10k users'],
            ['chaidex', '$100k+ liquidity managed'],
            ['sei ecosystem', '1M+ tokens moved · 600% developer growth'],
          ].map(([project, stat]) => (
            <div key={project} style={{ display: 'flex', gap: 12, fontSize: 12 }}>
              <span style={{ color: '#333', minWidth: 100 }}>{project}</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ['github', 'github.com/gitshreevatsa', 'https://github.com/gitshreevatsa'],
            ['linkedin', 'linkedin.com/in/shreyas-padmakiran', 'https://linkedin.com/in/shreyas-padmakiran'],
            ['x', 'x.com/sakai_thezkguy', 'https://x.com/sakai_thezkguy'],
            ['email', 'hey@shreyaspadmakiran.com', 'mailto:hey@shreyaspadmakiran.com'],
            ...
            <div key={label} style={{ display: 'flex', gap: 12, fontSize: 12 }}>
              ...
              <a
                ...
                style={{ color: '#555', textDecoration: 'underline', textUnderlineOffset: 3, textDecorationColor: '#2a2a2a' }}
              >
                {display}
              </a>
            </div>
...
function CloseButton({ onClose }: { onClose: () => void }) {
  return (
    <button
      onClick={onClose}
      style={{
        marginTop: 52, border: '1px solid #2a2a2a', background: 'transparent',
        color: '#666', padding: '10px 36px', fontSize: 13, cursor: 'pointer', letterSpacing: '0.08em',
      }}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') { setBuffer(''); return; }
      if (e.key === 'Backspace') { setBuffer((p) => p.slice(0, -1)); return; }
      if (e.key.length !== 1) return;
      setBuffer((prev) => {
        const next = (prev + e.key).toLowerCase();
        if ((COMMANDS as readonly string[]).includes(next)) {
          setTimeout(() => { setPanel(next as Command); setBuffer(''); }, 80);
          return next;
        }
        if (!COMMANDS.some((cmd) => cmd.startsWith(next))) return e.key.toLowerCase();
        return next;
      });
    };
    window.addEventListener('keydown', handler);
  ...
  return (
    <>
      ...
      >
        ...
        >
          {[
            { label: 'github',   sub: 'the code place', href: 'https://github.com/gitshreevatsa' },
            { label: 'linkedin', sub: 'the brag place', href: 'https://linkedin.com/in/shreyas-padmakiran' },
            { label: 'x',        sub: 'the rant place', href: 'https://x.com/sakai_thezkguy' },
          ].map((card) => (
            <a
              key={card.label}
              href={card.href}
              ...
              style={{
                ...
                border: '1px solid #1a1a1a', padding: '20px 22px',
                background: 'rgba(12,12,12,0.75)', backdropFilter: 'blur(4px)',
            ...
            >
              ...
              <span style={{ color: '#e0e0e0', fontSize: 16, fontWeight: 500 }}>{card.label}</span>
              <span style={{ color: '#4a4a4a', fontSize: 12, fontStyle: 'italic', fontFamily: 'var(--font-serif)' }}>
                {card.sub}
              </span>
            </a>
          ))}
      ...
      <div
        style={{
          ...
          padding: '0 24px',
          background: 'rgba(8,8,8,0.92)', borderTop: '1px solid #161616',
        ...
        <span style={{ color: '#444', fontSize: 12 }}>
          {buffer ? (
            <span style={{ color: '#777' }}>
              $ {buffer}
              <span style={{
                display: 'inline-block', width: 6, height: 12, background: '#666',
